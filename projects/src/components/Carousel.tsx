...
const Carousel = () => {
  ...
  const [showThumbnails, setShowThumnails] = useState(false);
  ...
  const selectSide = (index: number) => {
    ...
    window.setTimeout(() => setShowThumnails(false), 5000);
  };
  ...
  return (
    ...
    <div className="relative w-[50%] rounded-lg overflow-hidden">
      <motion.div
        className={`flex mb-[2rem] transition-opacity
            duration-300 ${
        ...
        {images.map((image, index) => (
            <div
            key={index}
            onClick={() => selectSide(index)}
            className={`w-[10rem] h-[5rem] mt-[2rem] cursor-pointer rounded-lg overflow-hidden
                border-2 ${
                    currentIndex === index
                    ? "border-blue-500"
                    : "border-transparent"
      ...
      <button
        onClick={prevSlide}
        className={`absolute left-4 transform text-white
      ...
      <button
        onClick={nextSlide}
        className={`absolute right-4 transform-translate-y-1/2 p-2 text-white
            shadow transition-opacity duration-300 ${
                isHovered ? "opacity-100" : "opacity-0"
            }`}
            >
        &#10095;
